...
expName = 'LetterRSVP_rareD'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\chris\\OneDrive\\Desktop\\NEW EXPERIMENTS\\PsychoPy Perceptual load task\\RSVP TASK\\LetterRSVP_rareD_lastrun.py',
...
trials = data.TrialHandler(nReps=1, method='random', 
    ...
    trialList=data.importConditions('RSVPtrials_rareA.xlsx'),
...
for thisTrial in trials:
    ...
    if LeftD == "FEAR":
        ...
        FearLeft_marker += 1
        if FearLeft_marker == distractor_presentations:
            FearLeft_marker = 0
    elif RightD == "FEAR":
        D_L = "BLANK.bmp"
        D_R = FearRight_trials[FearRight_marker]
        FearRight_marker += 1
        if FearRight_marker == distractor_presentations:
            FearRight_marker = 0
    elif LeftD == "NEUTRAL":
        D_R = "BLANK.bmp"
        D_L = NeutralLeft_trials[NeutralLeft_marker]
        NeutralLeft_marker += 1
        if NeutralLeft_marker == distractor_presentations:
            NeutralLeft_marker = 0
    elif RightD == "NEUTRAL":
        D_L = "BLANK.bmp"
        D_R =  NeutralRight_trials[NeutralRight_marker]
        NeutralRight_marker += 1
        if NeutralRight_marker == distractor_presentations:
            NeutralRight_marker = 0
    else:
        D_L = "BLANK.bmp"
        D_R = "BLANK.bmp"
